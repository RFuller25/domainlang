...
type gen struct {
	...
	// annotate wraps each node's emitted statements in marker comments, so the
	// lines a node compiled to can be found in the finished source. It is off
	// for a real build: the markers exist to be stripped (see emit), and a
	// compiler that emitted different text depending on who was watching would
	// be a compiler nobody could trust.
	annotate bool
	marks    []*ir.Node // the node each marker id stands for, id 1 first
	varn     int
	parsen   int
	...
	bindNames exprEnv
	// globals is every `Cursed Object` slot declared so far, in declaration
	// order, so a Part can save and restore the ones its body can reach. It is
	// not a scope: globals do not shadow and are never popped. See
	// codegen/globalgen.go.
	globals []globalSlot
}
...
type Options struct {
	...
	// Tuning is what a caller has measured about the input this binary will be
	// run on. The zero value is `domain build`'s own behaviour; see tuning.go
	// for why the compiler is allowed to hold it at all.
	Tuning Tuning
}

// Span is a half-open range of 1-based lines in emitted source: the code one
// pipeline node became.
type Span struct {
	Start int // first line of the node's code
...
func EmitProgram(p *ir.Pipeline, opts Options) (string, error) {
	src, _, err := emit(p, opts, false)
	return src, err
}

// EmitAnnotated compiles the pipeline exactly as EmitProgram does, and also
// reports which lines of the result each node became.
//
// It exists for the step-by-step visualizer, where "what Go did this stage turn
// into" is a question about one row of a trace rather than about the program.
// The mapping is built by emitting marker comments around each node's
// statements, formatting, and then reading the markers back out and removing
// them — so the line numbers are the formatted source's own, and the source
// handed back is byte-for-byte what a build would produce. A node the backend
// fused into its neighbour (see tryFuse) has no span of its own; a caller shows
// the whole program instead.
func EmitAnnotated(p *ir.Pipeline, opts Options) (string, map[*ir.Node]Span, error) {
	return emit(p, opts, true)
}
...
func emit(p *ir.Pipeline, opts Options, annotate bool) (string, map[*ir.Node]Span, error) {
	...
	// The collector settings, if the caller measured any. They are computed
	// here rather than at the point they are written because the import block
	// is emitted first, and a runtime/debug added after it would never appear.
	gcTuning := gcTuningStmt(opts.Tuning)
	if gcTuning != "" {
		g.imp("runtime/debug")
	}
	procsTuning := procsTuningStmt(opts.Tuning)
	if procsTuning != "" {
		g.imp("runtime")
	}
	// The probe helper is declared for every probe build, whether or not the
	// program has a binding to report: a probe run that finds nothing has to
	// be able to say so, and an empty report file says it.
	if opts.Tuning.ProbeConstants {
		g.helper("dmProbe", declConstProbe, "fmt", "os", "strings")
	}
	...
	// Both hooks are one environment lookup on an ordinary run. The profile
	// stops before the allocation figures are read, since defers unwind in
	// reverse order and the profiler should not be running while ReadMemStats
	// stops the world.
	// The collector settings come before anything else in main: after the first
	// allocation they would be adjusting a heap that had already grown under
	// the default policy, which is most of what they exist to avoid.
	probeReport := ""
	if opts.Tuning.ProbeConstants {
		probeReport = "\tdefer dmProbeReport()\n"
	}
	out.WriteString("\nfunc main() {\n" + gcTuning + procsTuning +
		"\tdefer dmAllocReport()\n\tdefer dmCPUProfile()()\n" + probeReport)
...
//
// gofmt is free to move code around — it re-indents, it aligns, it can drop a
// blank line — so line numbers counted while emitting are not the finished
// file's line numbers. A comment, on the other hand, stays exactly where it was
// written relative to the statements around it. So each node's statements are
// bracketed by a pair of marker comments carrying the node's id, and the spans
// are read back from the *formatted* text, which cannot be wrong about itself.
//
// Ids rather than emission order, because order does not survive assembly: a
// `Using:` block body is emitted into a top-level function (blockgen.go) that
// lands above main, ahead of markers written before it.
const markPrefix = "//domain:mark:"

// markNode brackets one node's statements, returning the call that closes the
// bracket. Without annotation both are nothing at all.
func (g *gen) markNode(n *ir.Node) func() {
	if !g.annotate || n == nil {
		return func() {}
	}
	g.marks = append(g.marks, n)
	id := len(g.marks)
	g.wl("%s%d", markPrefix, id)
	return func() { g.wl("%send:%d", markPrefix, id) }
}

// readMarks strips the marker comments back out and reports, for each node that
// emitted anything, the lines its code occupies in the returned source.
func (g *gen) readMarks(src string) (string, map[*ir.Node]Span) {
	...
	for _, line := range strings.Split(src, "\n") {
		...
		// A marker occupies no line of its own in the result: an opening one
		// says the node's code starts on the next line kept, a closing one that
		// it ended with the last line kept.
		body := strings.TrimPrefix(mark, markPrefix)
		if id, ok := strings.CutPrefix(body, "end:"); ok {
			g.closeMark(spans, starts, id, len(kept)+1)
			continue
		}
...
// closeMark records a node's span, unless the node emitted no lines at all — a
// Binding Vow under --release compiles to nothing, and a span of no lines would
// point a reader at whatever happened to follow it.
func (g *gen) closeMark(spans map[*ir.Node]Span, starts map[int]int, id string, end int) {
	n, err := strconv.Atoi(id)
	if err != nil {
		return
	}
	start, ok := starts[n]
	if !ok || start >= end || n > len(g.marks) {
		return
	}
	node := g.marks[n-1]
	if _, seen := spans[node]; seen {
		return // emitted more than once (an interned block body); the first wins
	}
	spans[node] = Span{Start: start, End: end}
}
...
func (g *gen) emitSequence(nodes []*ir.Node, in string) (string, error) {
	// Stages the caller measured doing nothing are removed before anything
	// else looks at the list, so the fuser sees the pipeline that will actually
	// be emitted. Filtering inside the loop instead would let an elided stage
	// be swallowed into a fused pair and emitted after all — correct, but the
	// elision would silently not happen.
	nodes = g.keepNodes(nodes)

	cur := in
	for i := 0; i < len(nodes); i++ {
		// The bracket goes around whichever lowering runs, fusion included: a
		// fused pair is attributed to the first of the two, which is the row a
		// reader of the trace would be looking at.
		endMark := g.markNode(nodes[i])
...
func (g *gen) emitNode(n *ir.Node, in string) (string, error) {
	...
	case "Cursed Object", "Cursed Tool":
...
func (g *gen) emitConvertToIntegers(n *ir.Node, in string) (string, error) {
	...
	// The elements come from a Split, so they carry no surrounding whitespace
	// and dmParseInt's TrimSpace is pure overhead. dmParseIntSeg is the same
	// function with the trim moved into its fallback, so a segment that does
	// need trimming still parses identically.
	...
	if n.In.Equal(ir.List(ir.Text())) {
		...
		g.wl("for %s, %s := range %s {", i, s, in)
		g.in()
		g.wl("%s[%s] = dmParseIntSeg(%s)", v, i, s)
		g.out()
		g.wl("}")
		return v, nil
	}
	...
	g.in()
	g.wl("%s[%s] = dmParseIntSeg(%s)", row, j, s)
	g.out()
	g.wl("}")
	g.wl("%s[%s] = %s", v, i, row)
	g.out()
...
func (g *gen) emitSort(n *ir.Node, in string) (string, error) {
	...
	lt, err := lessExpr(n.In.Elem, lo, hi)
	if err != nil {
...
func (g *gen) emitFilter(n *ir.Node, in string) (string, error) {
	...
	// Sized to the input, like Partition's halves: a predicate that keeps most
	// of its input otherwise pays ~65 reallocations on a two-million-element
	// list, because Go grows a large slice by a quarter at a time. The cost is
	// transient memory no larger than the list already in hand.
...
func (g *gen) emitEmit(n *ir.Node, in string) (string, error) {
	...
	printRaw, printFmt := "fmt.Println(%s)", "fmt.Println(%s(%s))"
	if target, _ := n.Meta["target"].(string); target == "stderr" {
		g.imp("os")
		printRaw, printFmt = "fmt.Fprintln(os.Stderr, %s)", "fmt.Fprintln(os.Stderr, %s(%s))"
	}
	switch n.In.Kind {
...
func (g *gen) emitPart(n *ir.Node, in string) (string, error) {
	...
	// Sibling Parts branch from one state, and that guarantee covers the
	// globals a Part can reach as well as the value it was handed — see
	// saveGlobals. This is the compiled half of eval.SnapshotGlobals.
	restoreGlobals, err := g.saveGlobals()
	if err != nil {
		return "", err
	}
