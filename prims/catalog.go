...
var Catalog = map[string]PrimDoc{
	...
	"Transpose": {"Transpose", "Cursed Technique", "Grid<T> → Grid<T> | List<List<T>> → List<List<T>>",
		"Swaps rows and columns; a ragged list of rows is an error.", "transpose"},
	...
	"Min By": {"Min By", "Maximum Technique", "List<T> × (T → K) → T",
		"The element whose ordered key is smallest (first wins ties).", "min-by--max-by"},
	"Max By": {"Max By", "Maximum Technique", "List<T> × (T → K) → T",
		"The element whose ordered key is largest (first wins ties).", "min-by--max-by"},
	...
	"Convert To Graph": {"Convert To Graph", "Channeled Energy", "List<(K,K)> | List<(K,K,Int)> | Map<K,List<K>> → Graph<K>",
	...
	"Explore": {"Explore", "Domain Expansion", "S × (S → List<S>) → List<S> | Int | Map<S,Int> | V",
		"Search over an implicit state graph — breadth-first, cheapest-first with a Cost:, or folded with Tally; the answer to a problem that seems to need recursion.", "explore"},
	...
	"Shortest Path": {"Shortest Path", "Domain Expansion", "Graph<K> × Start: × Goal: → List<K>",
		"The cheapest path between two nodes as a list of nodes, empty when unreachable.", "shortest-path"},
	"BFS": {"BFS", "Domain Expansion", "Grid<T> × (T → Bool) → Grid<Int> | Graph<K> × Start: → Map<K,Int>",
		"Breadth-first step distances from a start over walkable cells (unreachable = -1).", "bfs"},
	"Dijkstra": {"Dijkstra", "Domain Expansion", "Grid<Int> → Grid<Int> | Graph<K> × Start: → Map<K,Int>",
	...
	"Connected Components": {"Connected Components", "Domain Expansion", "Grid<T> × (T → Bool) → Int | Graph<K> → Int",
